...
# Verzeichnisse, die nie eigener Code sind. `check_ruff_config` und
# `list_tool_names` filtern sie laengst; `check_blind_assertions` tat es nicht
# und meldete deshalb in einem Repo mit lokalem `.venv/` sechs B3-ERROR aus
# `setuptools`- und `referencing`-Testsuiten. Das ist kein Fehlalarm, den man
# wegsieht: der Exit-Code 1 macht das Skript fuer genau die Routine unbrauchbar,
# die CONTRIBUTING.md vorschreibt. In der CI fiel es nicht auf, weil dort kein
# venv im Repo liegt — die Luecke war nur lokal sichtbar.
VENDOR_DIRS = {".venv", "venv", "env", "node_modules", ".git", "build", "dist"}
...
def check_blind_assertions(repo: Path, rep: Report) -> None:
    ...
    for py in sorted(repo.rglob("test_*.py")) + sorted(repo.rglob("*_test.py")):
        if any(part in VENDOR_DIRS for part in py.parts):
            continue
        raw = py.read_text(encoding="utf-8", errors="replace")
